...
const Features = props => {
  return (props.data?.length ? (
    <section className="bg-[#59544F]">
      <div className="text-xl md:text-2xl text-center p-5 font-extralight tracking-widest uppercase text-[#c8a16e]">Особенности композиции {props.title}</div>
      <div>
        {props.data.map((obj) => {
          return (
            <FeatureFigure
